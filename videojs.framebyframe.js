...
function assignId(stepSize) {
    return (stepSize > 0 ? 'forward' : 'backward');
}
...
function framebyframe(options) {
    ...
    player.ready(function() {
        options.steps.forEach(function(opt) {
            let buttonId = assignId(opt.step);    
                ...
                new FBFButton(player, {
                    el: videojs.createEl(
                        'button',
                        {
                            ...
                            innerHTML: '<div class="vjs-control-content" id='+buttonId+' style="font-size: 11px; line-height: 28px;"><span class="vjs-fbf">' + opt.text + '</span></div>'
